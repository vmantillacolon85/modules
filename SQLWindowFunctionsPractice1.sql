...
FROM sneaker_sales --will show all the rank from 1-10 in this example

--compare only the departments

SELECT employee_name, department, revenue,
RANK () OVER(PARTITION BY department ORDER BY revenue DESC) as SalesRank
FROM sneaker_sales --shows the leaders within each department, so there are multiples #1s because there are multiple departments

SELECT employee_name, department, revenue,
RANK () OVER(PARTITION BY department ORDER BY revenue DESC) as sales_rank_by_dept
FROM sneaker_sales 
ORDER BY 4 --will show all the #1s and #2s and #3s together
...
SELECT employee_name, department, revenue,
DENSE_RANK() OVER(PARTITION BY department ORDER BY revenue DESC) AS sales_rank_by_dept
FROM sneaker_sales
WHERE department = 'kids' /*Dense rank will result in the table not skipping over the next number 
--like in a tie with a rank. so here 1, 2,2,3 are respective of their positions where as 
--before Isabella was relegated to number 4 because of rank counting Liam and James as 2 before her 
--because Liam and James were tied. */
...
-------more complexity, that shows the ranks in each department 
...
SELECT employee_name, department, revenue,
CUME_DIST () OVER(ORDER BY revenue DESC) AS percentile_sales_rank_by_dept
FROM sneaker_sales
--WHERE department = 'kids'

-----------------------LAG-------------------
--Comparing side by side to show previous figures
...
--------------LEAD----------------
----Comparing side by side to show the next figures
